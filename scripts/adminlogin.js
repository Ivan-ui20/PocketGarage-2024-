...
if (nextBtn) {
  nextBtn.addEventListener("click", function () {
    ...
    const signupPhoneErrorMsg = document.getElementById(
    ...
    if (signupPhoneField.value.length !== 11) {
      ...
      signupPhoneErrorMsg.textContent =
        "Please enter a valid 11-digit phone number."; // Set error message
    } else {
      signupPhoneField.style.borderColor = ""; // Reset border color if valid
    }

    // Validate password field (Check if it's filled)
    const signupPasswordErrorMsg = document.getElementById(
